...
__forceinline__ __device__ void floyd_warshall_kernel(float* matrix, int Y, int X, int y_c, int x_c, int y_a, int x_a, int y_b, int x_b, bool ph3){

    const unsigned int moveA = dnV*(y_a + Y) + x_a;
    const unsigned int moveB = y_b*dnV + x_b + X;

    const unsigned int posC = dnV*(y_c+Y) + x_c + X;

    float sum = matrix[posC];
    float dAB;

    const unsigned int limity = dnV - y_b;
    const unsigned int limitx = dnV - x_a;

    if(!ph3){
        for(unsigned int k = 0; k < dBS && k < limitx && k < limity; k++){
            dAB = matrix[moveA + k] + matrix[moveB + k*dnV];
            if(dAB != dINF && sum > dAB){
                sum = dAB;
                matrix[posC] = sum;}
            __syncthreads();
...
__global__ void block_ph1(float* matrix, int pbid){
    ...
    if(by != pbid || bx != pbid || pb_point + Y >= dnV || pb_point + X >= dnV)    // se si dovesse trovare sulla diagonale
        return;

    floyd_warshall_kernel(matrix, Y, X, pb_point, pb_point, pb_point, pb_point, pb_point, pb_point, false);
} 
...
__global__ void block_ph2(float* matrix, int pbid){
    ...
    if(bx == 0 && by_point + Y < dnV && pb_point + X < dnV)
        floyd_warshall_kernel(matrix, Y, X, by_point, pb_point, by_point, pb_point, pb_point, pb_point, false); // C, C, B
    if(by == 0 && pb_point + Y < dnV && bx_point + X < dnV)
        floyd_warshall_kernel(matrix, Y, X, pb_point, bx_point, pb_point, pb_point, pb_point, bx_point, false); // C, A, C
}
...
__global__ void block_ph3(float* matrix, int pbid){
    ...
    if(bx == pbid || by == pbid || bx_point + X >= dnV || by_point + Y >= dnV)
        return;

    floyd_warshall_kernel(matrix, Y, X, by_point, bx_point, by_point, pb_point, pb_point, bx_point, true);
}
...
__host__ int main(int argc, char* argv[]) {
    ...
    const unsigned int nblock =  (nV+BLOCK_SIZE-1)/BLOCK_SIZE;
